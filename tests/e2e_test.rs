...
fn test_e2e_large_scale_100mb() {
    ...
    let original_hashes = hash_directory(&source_dir);
    println!("Hashing took {:?} for {} files", start_hash.elapsed(), original_hashes.len());
...
fn test_e2e_incremental_backup() {
    ...
    println!("V2 has {} files (added {})", hashes_v2.len(), hashes_v2.len() - hashes_v1.len());
    ...
    println!("Second backup (incremental) took {:?}", start_backup2.elapsed());
    println!("Backup output: {}", String::from_utf8_lossy(&backup2.stdout));
    ...
    println!("Third backup (modified files) took {:?}", start_backup3.elapsed());
    ...
    assert_eq!(hashes_v3.len(), restored_hashes.len(), 
               "File count mismatch: expected {}, got {}", hashes_v3.len(), restored_hashes.len());
    
    for (name, expected) in &hashes_v3 {
        let actual = restored_hashes.get(name)
