...
fn create_test_client(repo_path: &str) -> Option<Pan123Client> {
    ...
    Some(Pan123Client::new(client_id, client_secret, repo_path.to_string()))
}
...
async fn test_cache_scenario1_basic_cache_hit() {
    ...
    let dir_id = match client.ensure_path(&repo_path).await {
        Ok(id) => id,
        Err(e) => {
            eprintln!("Failed to create test directory (may be rate limited): {:?}", e);
            return;
        }
    };
    
    // First call - should fetch from API and cache
    let files1 = client.list_files(dir_id).await.expect("First list_files failed");
    
    // Second call - should use cache (we can verify by checking debug logs or timing)
    let files2 = client.list_files(dir_id).await.expect("Second list_files failed");
    
    // Results should be identical
    assert_eq!(files1.len(), files2.len(), "Cache should return same number of files");
...
async fn test_cache_scenario2_upload_new_file() {
    ...
    let file_id = client.upload_file(dir_id, "test-file.txt", test_data.clone())
        .await
        .expect("upload_file failed");
    
    // List files again - should include new file from cache
    let files_after = client.list_files(dir_id).await.expect("list_files after upload failed");
    
    assert_eq!(files_after.len(), 1, "Should have exactly one file after upload");
    assert_eq!(files_after[0].filename, "test-file.txt", "Filename should match");
    assert_eq!(files_after[0].size, test_data.len() as i64, "Size should match");
...
async fn test_cache_scenario3_overwrite_upload() {
    ...
    let file_id_v1 = client.upload_file(dir_id, "config", data_v1.clone())
        .await
        .expect("first upload failed");
    
    let files_v1 = client.list_files(dir_id).await.expect("list after v1 failed");
    assert_eq!(files_v1.len(), 1, "Should have one file after first upload");
    assert_eq!(files_v1[0].size, data_v1.len() as i64, "Size should match v1");
    ...
    let file_id_v2 = client.upload_file(dir_id, "config", data_v2.clone())
        .await
        .expect("second upload failed");
    
    let files_v2 = client.list_files(dir_id).await.expect("list after v2 failed");
    
    // Should still have exactly one file (not duplicated)
    assert_eq!(files_v2.len(), 1, "Should still have exactly one file after overwrite");
    assert_eq!(files_v2[0].filename, "config", "Filename should be unchanged");
    assert_eq!(files_v2[0].size, data_v2.len() as i64, "Size should be updated to v2");
    assert_eq!(files_v2[0].file_id, file_id_v2, "File ID should be updated");
    assert_ne!(file_id_v1, file_id_v2, "File IDs should differ between versions");
...
async fn test_cache_scenario4_delete_removes_from_cache() {
    ...
    let file_id = client.upload_file(dir_id, "to_delete.txt", test_data)
        .await
        .expect("upload_file failed");
    
    // Verify file is in cache
    let files_before = client.list_files(dir_id).await.expect("list before delete failed");
    assert_eq!(files_before.len(), 1, "Should have one file before delete");
    
    // Delete the file
    client.delete_file(dir_id, file_id).await.expect("delete_file failed");
    
    // Verify file is removed from cache
    let files_after = client.list_files(dir_id).await.expect("list after delete failed");
    assert!(files_after.is_empty(), "Cache should be empty after delete");
    
    // Verify find_file returns None
    let found = client.find_file(dir_id, "to_delete.txt").await.expect("find_file failed");
    assert!(found.is_none(), "find_file should return None for deleted file");
...
async fn test_cache_scenario5_idempotent_delete() {
    ...
    let file_id = client.upload_file(dir_id, "existing.txt", test_data)
        .await
        .expect("upload_file failed");
    ...
    let files_after = client.list_files(dir_id).await.expect("list after failed delete");
    assert_eq!(files_after.len(), 1, "Cache should still have one file");
    assert_eq!(files_after[0].filename, "existing.txt", "Original file should be intact");
...
async fn test_cache_scenario6_multi_directory_isolation() {
    ...
    let _ = client.list_files(dir_a_id).await.expect("list dir_a failed");
    let _ = client.list_files(dir_b_id).await.expect("list dir_b failed");
    
    // Upload to dir_a
    let data_a = Bytes::from("file in dir_a");
    let file_a_id = client.upload_file(dir_a_id, "file_a.txt", data_a)
        .await
        .expect("upload to dir_a failed");
    
    // Upload to dir_b
    let data_b = Bytes::from("file in dir_b");
    let file_b_id = client.upload_file(dir_b_id, "file_b.txt", data_b)
        .await
        .expect("upload to dir_b failed");
    
    // Verify isolation
    let files_a = client.list_files(dir_a_id).await.expect("list dir_a after upload");
    let files_b = client.list_files(dir_b_id).await.expect("list dir_b after upload");
    
    assert_eq!(files_a.len(), 1, "dir_a should have 1 file");
    assert_eq!(files_b.len(), 1, "dir_b should have 1 file");
    assert_eq!(files_a[0].filename, "file_a.txt", "dir_a should have file_a.txt");
    assert_eq!(files_b[0].filename, "file_b.txt", "dir_b should have file_b.txt");
    
    // Upload more to dir_a, should not affect dir_b
    let data_a2 = Bytes::from("another file in dir_a");
    let file_a2_id = client.upload_file(dir_a_id, "file_a2.txt", data_a2)
        .await
    ...
    assert_eq!(files_b_after.len(), 1, "dir_b should still have 1 file (not polluted)");
...
async fn test_cache_scenario7_upload_without_cache_init() {
    ...
    let file_id = client.upload_file(dir_id, "first.txt", test_data.clone())
        .await
        .expect("upload_file failed");
...
async fn test_cache_scenario8_rapid_consecutive_operations() {
    ...
    let file_a_id = client.upload_file(dir_id, "a.txt", data_a)
        .await
        .expect("upload a failed");
    
    let data_b = Bytes::from("file b");
    let file_b_id = client.upload_file(dir_id, "b.txt", data_b)
        .await
        .expect("upload b failed");
    
    client.delete_file(dir_id, file_a_id).await.expect("delete a failed");
    
    let data_c = Bytes::from("file c");
    let file_c_id = client.upload_file(dir_id, "c.txt", data_c)
        .await
    ...
    assert!(!filenames.contains(&"a.txt"), "Should NOT contain a.txt (deleted)");
